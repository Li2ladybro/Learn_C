...
int main()
{
	...
	Button btn = { 40,100,200,240,0,RGB(144,65,237),GREEN,"blue",20,1,_T("Entre"),40,20,45};	
	//带边框,且填充内部颜色，还加了边框色,设置了边角的半径
	out_long(btn);  
	while(1)
	{		 
		if (!(mouse(btn, 10)))
...
int mouse(Button btn,int change)
{
   ...
   switch (m.message)  
   {
      ...
      if (m.x >= btn.left && m.x <= btn.right&&m.y >= btn.top && m.y <= btn.bottom)
	    ...
	    btn.bottom +=change;
	    out_long(btn);
		mciSendString(_T("open res\\指尖笑 - 淤泥里的花.mp3 alias music"), 0, 0, 0);     //alias是别名的意思，下面就可以去直接用music来代替kk.mp3
